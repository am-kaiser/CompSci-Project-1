...
def apply_regression(p, n, noise, r=np.zeros(1), reg_type="ols", ridge_lambda=np.ones(1), n_boots=np.ones(1, dtype=int),
    ...
    # bias in test set
    test_bias_arr = np.zeros([len(p), len(n), len(noise), len(r), len(ridge_lambda), len(n_boots), len(k_folds)])
    # variance in test set
    test_var_arr = np.zeros([len(p), len(n), len(noise), len(r), len(ridge_lambda), len(n_boots), len(k_folds)])
                ...
                for l in range(len(r)):
                    if (reg_type == "ols"):
                        ...
                        for rl in range(len(ridge_lambda)):
                            linear_reg.apply_leastsquares(order=p[i], test_ratio=r[l], ridge=True,
                                                          lmbda=ridge_lambda[rl])
                        ...
                        for rl in range(len(ridge_lambda)):
                            for b in range(len(n_boots)):
                                linear_reg.apply_leastsquares_bootstrap(order=p[i], test_ratio=r[l], n_boots=n_boots[b],
                                                                        ridge=True, lmbda=ridge_lambda[rl])
                        ...
                        for rl in range(len(ridge_lambda)):
                            for c in range(len(k_folds)):
                                linear_reg.apply_leastsquares_crossvalidation(order=p[i], kfolds=k_folds[c], ridge=True,
                                                                              lmbda=ridge_lambda[rl])
...
def apply_regression2(p, n, noise, r=np.zeros(1), reg_type="ols", ridge_lambda=np.ones(1),
