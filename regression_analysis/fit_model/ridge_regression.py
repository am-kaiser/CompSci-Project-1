...
def perform_RR(input_x1, input_x2, obs_var, order, lam, train_frac):
    """
    ...
    resp_var_RR_test = np.dot(D_test, beta_RR_train).reshape(y_test.shape[0], y_test.shape[1])

    # Step 4: Calculate errors
    # Calculate error evaluaters for the test dataset
    error_class = basis_functionality.Error_Measures(y_test, resp_var_RR_test)
...
def perform_RR_cross_val(input_x1, input_x2, obs_var, order, lam, num_fold=5):
    """
    Performs ridge regression for a 2D function using polynomials of order "order".
    ...
    for fold_index in range(num_fold):
        ...
        # Get RR response variable for test dataset
        regress_obj_test = basis_functionality.Design_Matrix_2D(x1_test, x2_test, y_test, order)
        D_test = regress_obj_test.make_design_matrix()
        resp_var_RR_test = np.dot(D_test, beta_RR_train).reshape(y_test.shape[0], y_test.shape[1])
...
def perform_RR_bootstrap(input_x1, input_x2, obs_var, order, lam, train_frac=0.8, num_boot=5):
    """
    Performs ridge regression for a 2D function using polynomials of order "order".
    ...
    for boot_index in range(num_boot):
        ...
        # Calculate beta and response variable for train dataset
        beta_RR_train, resp_var_RR_train = calculate_RR(x1_train_boot, x2_train_boot, y_train_boot, order, lam=lam)
        # Calculate error evaluaters for the train dataset
        error_class = basis_functionality.Error_Measures(y_train_boot, resp_var_RR_train)
...
if __name__ == "__main__":
    ...
    for i, order in enumerate(orders):
        MSE_train[:, i], MSE_test[:, i], R2_train[:, i], R2_test[:, i] = perform_RR(input_x1, input_x2, obs_var, order,
                                                                                    lam=0.2, train_frac=0.8)
        MSE_train_c[:, i], MSE_test_c[:, i], R2_train_c[:, i], R2_test_c[:, i] = perform_RR_cross_val(input_x1,
                                                                                                      input_x2,
                                                                                                      obs_var,
                                                                                                      order,
                                                                                                      lam=0.2,
                                                                                                      num_fold=5)
        MSE_train_b[:, i], MSE_test_b[:, i], R2_train_b[:, i], R2_test_b[:, i] = perform_RR_bootstrap(input_x1,
                                                                                                      input_x2,
                                                                                                      obs_var,
                                                                                                      order,
                                                                                                      lam=0.2,
