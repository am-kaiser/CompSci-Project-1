...
def create_0(test_ratios, l2_lambda, k_folds, learn_rate, num_min_batch, epochs):
    """Create array filled with zeros."""
    return np.zeros([len(test_ratios), len(l2_lambda), len(k_folds), len(learn_rate), len(num_min_batch),
                     len(epochs)])


def apply_regression(test_ratios=np.zeros(1), reg_type="logistic_sgd", l2_lambda=np.ones(1), k_folds=np.ones(1, dtype=int),
                     learn_rate=np.ones(1), num_min_batch=np.ones(1), epochs=np.ones(1)):
    # create empty numpy arrays for train and test accuracy
    train_acc_arr = create_0(test_ratios, l2_lambda, k_folds, learn_rate, num_min_batch, epochs)
    test_acc_arr = create_0(test_ratios, l2_lambda, k_folds, learn_rate, num_min_batch, epochs)
                ...
                for epoch_ind, epoch in enumerate(epochs):
                    for learn_ind, learn_rat in enumerate(learn_rate):
                        for batch_ind, num_batch in enumerate(num_min_batch):
                            ...
                            train_acc_arr[ratio_ind, lam_ind, 0, learn_ind, batch_ind, epoch_ind] = logistic_reg.train_accuracy
                            test_acc_arr[ratio_ind, lam_ind, 0, learn_ind, batch_ind, epoch_ind] = logistic_reg.test_accuracy

        if reg_type == "logistic_scikit":
            logistic_reg.apply_logistic_regression(test_ratio=test_ratio, reg_method="logistic_scikit")
            train_acc_arr[ratio_ind, 0, 0, 0, 0, 0] = logistic_reg.train_accuracy
            test_acc_arr[ratio_ind, 0, 0, 0, 0, 0] = logistic_reg.test_accuracy
    ...
    for fold_ind, k_fold in enumerate(k_folds):
                ...
                for epoch_ind, epoch in enumerate(epochs):
                    for learn_ind, learn_rat in enumerate(learn_rate):
                        for batch_ind, num_batch in enumerate(num_min_batch):
                            ...
                            train_acc_arr[
                                0, lam_ind, fold_ind, learn_ind, batch_ind, epoch_ind] = logistic_reg.train_accuracy
                            test_acc_arr[0, lam_ind, fold_ind, learn_ind, batch_ind, epoch_ind] = logistic_reg.test_accuracy
        ...
        if reg_type == "logistic_scikit_crossvalidation":
            ...
            train_acc_arr[0, 0, fold_ind, 0, 0, 0] = logistic_reg.train_accuracy
            test_acc_arr[0, 0, fold_ind, 0, 0, 0] = logistic_reg.test_accuracy
    ...
    return train_acc_arr, test_acc_arr
